...
tuple<string,string> __hide(){
	...
	int repeatBlock = 0;
	int total = 0;
	vector<int> repeatBlocks(vSTRl.size());
	bool flag = 0;
	...
	for(int i = 0; i < vSTRm.size(); i++){
		...
		for(int j = 0; j < N_PROBLACION; j++){
			...
			if(repeat[choseOne] == 0){
				//repeatBlocks[choseOne] = repeatBlocks[choseOne] + 1;
				//total++;
				repeat[choseOne] = 1;
	...
	cout<<"\t\tRepeat Blocks->"<<repeatBlock<<endl;
	//cout<<"\t\tPromedio de bloques escondidos->"<<promedio(repeatBlocks)<<endl;
